...
def main():
    ...
    # Logging.
    debug_log_path = 'prl_joai_add_harvests.debug.log'
    ...
    parser = argparse.ArgumentParser(description='Add scheduled harvests to jOAI via the command line.')
    ...
    Harvest every {} days at {}\n'''.format(row['Repository name'],
                ...
                if interactive_shell_enabled:
                    prompt = '[a]dd / [y]es to all / [s]kip / [h]elp / [q]uit => '
                    action_choice = input(prompt)
                else:
                    action_choice = 'a'
                
                # Handle the help message and user errors.
                while action_choice not in ['a', 'y', 's', 'q']:
                    if action_choice == 'h':
